...
// HasConfirmedOrders applies the HasEdge predicate on the "confirmed_orders" edge.
func HasConfirmedOrders() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasConfirmedOrdersWith applies the HasEdge predicate on the "confirmed_orders" edge with a given conditions (other predicates).
func HasConfirmedOrdersWith(preds ...predicate.Order) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newConfirmedOrdersStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
