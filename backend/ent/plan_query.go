...
func (_q *PlanQuery) QueryGroups() *GroupQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, plan.GroupsTable, plan.GroupsPrimaryKey...),
		)
...
func (_q *PlanQuery) QueryOrders() *OrderQuery {
...
func (_q *PlanQuery) WithGroups(opts ...func(*GroupQuery)) *PlanQuery {
	query := (&GroupClient{config: _q.config}).Query()
...
func (_q *PlanQuery) WithOrders(opts ...func(*OrderQuery)) *PlanQuery {
	query := (&OrderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PlanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Plan, error) {
	...
	if query := _q.withGroups; query != nil {
		if err := _q.loadGroups(ctx, query, nodes,
			func(n *Plan) { n.Edges.Groups = []*Group{} },
			func(n *Plan, e *Group) { n.Edges.Groups = append(n.Edges.Groups, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withOrders; query != nil {
		if err := _q.loadOrders(ctx, query, nodes,
			func(n *Plan) { n.Edges.Orders = []*Order{} },
			func(n *Plan, e *Order) { n.Edges.Orders = append(n.Edges.Orders, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PlanQuery) loadGroups(ctx context.Context, query *GroupQuery, nodes []*Plan, init func(*Plan), assign func(*Plan, *Group)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Plan)
	nids := make(map[int64]map[*Plan]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(plan.GroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(plan.GroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(plan.GroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(plan.GroupsPrimaryKey[0]))
...
func (_q *PlanQuery) loadOrders(ctx context.Context, query *OrderQuery, nodes []*Plan, init func(*Plan), assign func(*Plan, *Order)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Plan)
	for i := range nodes {
	...
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(plan.OrdersColumn), fks...))
	}))
