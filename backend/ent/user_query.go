...
type UserQuery struct {
	...
	withConfirmedOrders       *OrderQuery
...
// QueryConfirmedOrders chains the current query on the "confirmed_orders" edge.
func (_q *UserQuery) QueryConfirmedOrders() *OrderQuery {
	query := (&OrderClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, selector),
			sqlgraph.To(order.Table, order.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, user.ConfirmedOrdersTable, user.ConfirmedOrdersColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withConfirmedOrders:       _q.withConfirmedOrders.Clone(),
...
// WithConfirmedOrders tells the query-builder to eager-load the nodes that are connected to
// the "confirmed_orders" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithConfirmedOrders(opts ...func(*OrderQuery)) *UserQuery {
	query := (&OrderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withConfirmedOrders = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withConfirmedOrders; query != nil {
		if err := _q.loadConfirmedOrders(ctx, query, nodes,
			func(n *User) { n.Edges.ConfirmedOrders = []*Order{} },
			func(n *User, e *Order) { n.Edges.ConfirmedOrders = append(n.Edges.ConfirmedOrders, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadConfirmedOrders(ctx context.Context, query *OrderQuery, nodes []*User, init func(*User), assign func(*User, *Order)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(order.FieldConfirmedBy)
	}
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ConfirmedOrdersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
