...
func (_q *OrderQuery) QueryPlan() *PlanQuery {
...
func (_q *OrderQuery) QueryConfirmedByUser() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *OrderQuery) WithPlan(opts ...func(*PlanQuery)) *OrderQuery {
	query := (&PlanClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) WithConfirmedByUser(opts ...func(*UserQuery)) *OrderQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := _q.withConfirmedByUser; query != nil {
		if err := _q.loadConfirmedByUser(ctx, query, nodes, nil,
			func(n *Order, e *User) { n.Edges.ConfirmedByUser = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OrderQuery) loadPlan(ctx context.Context, query *PlanQuery, nodes []*Order, init func(*Order), assign func(*Order, *Plan)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Order)
	for i := range nodes {
		fk := nodes[i].PlanID
...
func (_q *OrderQuery) loadConfirmedByUser(ctx context.Context, query *UserQuery, nodes []*Order, init func(*Order), assign func(*Order, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Order)
	for i := range nodes {
...
func (_q *OrderQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withConfirmedByUser != nil {
			_spec.Node.AddColumnOnce(order.FieldConfirmedBy)
		}
	}
